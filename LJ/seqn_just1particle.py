...
class Learner(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        for i in range(9):
            X_train = torch.Tensor(just_a_sequence(7).sequence[0:n]).to(device)
            #for j in range(258):
            #    dataset.append(X_train[:, j, :])
            #X_train = torch.rand((2,258,32))
            dataset.append(X_train[:,33])
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range (len(batch)):
            ...
            one_loss = nn.L1Loss(reduction='sum')(y_hat, x)
            ...
            loss = loss + one_loss# + 0.01*l1_reg
    ...
    def val_dataloader(self):
        ...
        for i in range(1):
            X_val = torch.Tensor(just_a_sequence(7).sequence[0:4]).to(device)
            dataset.append(X_val[:,33])
...
class f1(nn.Module):
    def __init__(self, encoding_size):
        ...
        embed1 = 128
        embed2 = 256
        self.func1 = MLP(encoding_size, embed1, hidden_layer=4, hidden_dim=256, activation='softplus')
        self.func2 = MLP(embed1, embed2, hidden_layer=3, hidden_dim=512, activation='softplus')
...
def main():
    ...
    parser.add_argument('--cp_dir', default='./model_ckpt/sequential_network_velocities_1particle')
