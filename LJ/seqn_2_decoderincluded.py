...
def build_model(args, ckpt=None):
    ...
    model = ODEBlock(func, encoding_size)
...
class Learner(pl.LightningModule):
    ...
    def forward(self, x, integration_time):
        ...
        x_ode, x_pos = self.model(x, integration_time)
        #x = self.mlp2(x)
        return x_ode, x_pos
    ...
    def train_dataloader(self):
        ...
        for i in range(9):
            ...
            X_pos = torch.Tensor(just_a_sequence(i).positions[0:n]).to(device)
            #for j in range(258):
            #    dataset.append(X_train[:, j, :])
            #X_train = torch.rand((2,258,32))
            dataset.append({'X_train': X_train, 'X_pos': X_pos})
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range (batch_len):
            ...
            y_hat, pos_hat = self.model(x[0], integration_time)

            ode_loss = nn.MSELoss()(y_hat, x)

            pos_loss = nn.MSELoss()(pos_hat, pos)
            ...
            loss = loss + ode_loss + pos_loss


            self.log('training loss', loss, on_step=True, prog_bar=True, logger=True)
            self.log('ode loss', ode_loss, on_step=True, prog_bar=True, logger=True)
    ...
    def val_dataloader(self):
        ...
        for i in range(1):
            ...
            X_pos = torch.Tensor(just_a_sequence(9).positions[0:499]).to(device)
            dataset.append({'X_val': X_val, 'X_pos': X_pos})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            loss = loss + one_loss + pos_loss

            #loss = loss/x.size()[0]

            self.log('val loss', loss, on_step=True, prog_bar=True, logger=True)
            self.log('pos val loss', pos_loss, on_step=True, prog_bar=True, logger=True)
            self.log('cord val loss', one_loss, on_step=True, prog_bar=True, logger=True)
    ...
    def ode_embed_func(self, embedding_start: torch.Tensor, t):
        ...
        y_hat, pos_fin = self.model(x, integration_time)
        return y_hat, pos_fin
...
class f1(nn.Module):
    def __init__(self, encoding_size):
        ...
        embed1 = 128
        embed2 = 256
        self.func1 = MLP(encoding_size, embed1, hidden_layer=4, hidden_dim=128, activation='softplus')
        self.func2 = MLP(embed1, embed2, hidden_layer=3, hidden_dim=258, activation='softplus')
        self.func3 = MLP(embed2, encoding_size, hidden_layer=4, hidden_dim=128, activation='softplus')
...
class ODEBlock(nn.Module):
    ...
    def __init__(self, odefunc, encoding_size):
        super(ODEBlock, self).__init__()
        self.odefunc = odefunc
        self.linear = MLP(encoding_size, 3, hidden_layer=4, hidden_dim=256, activation='leaky_relu')
    ...
    def forward(self, x, integration_time):
        ...
        #print(out[1].size())
        ode_result = out[1]
        pos_result = self.linear(ode_result)
        return ode_result, pos_result
        #return out
...
def main():
    ...
    parser.add_argument('--cp_dir', default='./model_ckpt/sequential_network_all_together')
    parser.add_argument('--state_ckpt_dir', default=None, type=str)
    parser.add_argument('--batch_size', default=1, type=int)
    parser.add_argument('--encoding_size', default=16, type=int)
