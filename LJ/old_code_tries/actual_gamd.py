...
from nn_module import SimpleMDNetNew_GAMD
...
def build_model(args, ckpt=None):
    ...
    print(param_dict)
    model = SimpleMDNetNew_GAMD(**param_dict)
...
class ParticleNetLightning_GAMD(pl.LightningModule):
    def __init__(self, args, num_device=1, epoch_num=100, batch_size=32, learning_rate=3e-4, log_freq=1000,
                 model_weights_ckpt=None, scaler_ckpt=None):
        super(ParticleNetLightning_GAMD, self).__init__()
    ...
    def predict_forces(self, pos: np.ndarray, verbose=False):
        ...
        forces, pos, emb = self.pnet_model([pos],
                               [edge_idx_tsr],
                               )
        force_end = time.time()
        if verbose:
        ...
        forces = self.denormalize(forces, self.training_var, self.training_mean)

        return pos, forces, emb
    ...
    def training_step(self, batch, batch_nb):
        pos_lst = batch['pos']
        gt_force_lst = batch['forces']
        edge_idx_lst = []
        for b in range(len(gt_force_lst)):
            pos, gt = pos_lst[b], gt_force_lst[b]

            pos = np.mod(pos, BOX_SIZE)

            gt_force = self.scale_force(gt, self.train_data_scaler).cuda()
            pos_lst[b] = torch.from_numpy(pos).float().cuda()
            gt_force_lst[b] = gt_force
        ...
        gt_forces = torch.cat(gt_force_lst, dim=0)
        gt_pos = torch.cat(pos_lst, dim=0)
        pos_lst = [pos + torch.randn_like(pos) * 0.005 for pos in pos_lst]

        forces, pos, emb = self.pnet_model(pos_lst,
                               edge_idx_lst,
                               )

        if self.loss_fn == 'mae':
            force_loss = nn.L1Loss()(forces, gt_forces)
            pos_loss = nn.L1Loss()(pos, gt_pos)
        else:
            force_loss = loss = nn.MSELoss()(forces, gt_forces)
            pos_loss = nn.L1Loss()(pos, gt_pos)

        conservative_loss = (torch.mean(forces)).abs()
        force_loss = force_loss + LAMBDA2 * conservative_loss
        ...
        self.log('force loss', force_loss, on_step=True, prog_bar=True, logger=True)
        self.log('pos loss', pos_loss, on_step=True, prog_bar=True, logger=True)
    ...
    def validation_step(self, batch, batch_nb):
        with torch.no_grad():
            ...
            loss = pos_loss+force_loss

            self.training_mean = self.train_data_scaler.mean_
            self.training_var = self.train_data_scaler.var_

            self.log('var force loss', force_loss, on_step=True, prog_bar=True, logger=True)
            self.log('var pos loss', pos_loss, on_step=True, prog_bar=True, logger=True)
            self.log('var total loss', loss, on_step=True, prog_bar=True, logger=True)
            #self.log(f'{self.loss_fn} loss', loss, on_step=True, prog_bar=True, logger=True)
            self.log('var', np.sqrt(self.training_var), on_step=True, prog_bar=True, logger=True)
...
class ModelCheckpointAtEpochEnd(pl.Callback):
    ...
    def on_epoch_end(self, trainer: pl.Trainer, pl_module: ParticleNetLightning_GAMD):
...
def train_model(args):
    ...
    model = ParticleNetLightning_GAMD(epoch_num=max_epoch,
...
def main():
    ...
    parser.add_argument('--cp_dir', default='./model_ckpt/GAMD_like')
    ...
    parser.add_argument('--disable_rotate_aug', dest='rotate_aug', default=False, action='store_false')
